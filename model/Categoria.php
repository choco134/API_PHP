...
class Categoria extends Conectar{
    ...
    public function insert_categoria($cat_nom,$cat_obs){
        ...
        $sql = "INSERT INTO categorias (cat_id, cat_nombre, cat_obs, est) VALUES (NULL, ?, ?, '1');";
        $sql = $conectar->prepare($sql);
    ...
    public function update_categoria($cat_id,$cat_nom,$cat_obs){
        ...
        $sql = "UPDATE categorias SET cat_nombre = ?, cat_obs = ? WHERE cat_id = ?;";
        $sql = $conectar->prepare($sql);
        $sql->bindValue(1, $cat_nom);
        $sql->bindValue(2, $cat_obs);
    ...
    $sql = "UPDATE categorias SET est = 0 WHERE cat_id = ?";
    $sql = $conectar->prepare($sql);
    $sql->bindValue(1, $cat_id);
    $sql->execute();
    return $resultado = $sql->fetchAll(PDO::FETCH_ASSOC);
